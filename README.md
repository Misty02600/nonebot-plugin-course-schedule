...
| course_reminder_enabled  | 否   | 是否启用课前提醒，默认为 `True`                                                 |
| course_reminder_offset   | 否   | 课前提醒提前量（分钟），默认 `30`                                               |
| course_reminder_interval | 否   | 上课提醒检查间隔（分钟），默认 `5`，此值不应大于60 或 course_reminder_offset。  |
...
| `绑定群聊`              | 让自己显示在本群的课表中。                                                         |
| `课表 <日期>`   | 显示你自己今天（或查询日期）接下来要上的课程。                                     |
| `群课表 <日期>` | 显示群里所有成员当前正在上或下一节要上的课程（或查询日期将要上的课）。             |
| `上课排行`              | 显示本周群友上课时长和节数的排行榜。                                               |
| `课前提醒`              | (自动触发) 插件会根据配置在课前自动发送提醒，无需手动命令。                        |

其中 `课表` / `群课表` 参数支持偏移数字和自然语言描述：

- 天数偏移：`3`、`-1`
- 标准日期：`2026-04-02`、`2026.04.02`
